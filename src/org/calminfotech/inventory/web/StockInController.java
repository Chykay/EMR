...
public class StockInController extends AbstractBaseController {
	...
	public String indexAction(ModelMap model,
		...
		if (!authorize.isAllowed("INVVESI00")) {
		...
		return displayStockInBatchesListPageTop100(stockInSearchForm, model);
	}

	private void loadStockInSearchForm(DateSearchForm stockInSearchForm,
			ModelMap model) {
	...
	private String displayStockInBatchesListPage(
			DateSearchForm stockInSearchForm, ModelMap model) {

		List<StockIn> stockInBatches = null;
		try {
			stockInBatches = this.stockInManager
					.getStockInBatchesList(stockInSearchForm);
	...
	private String displayStockInBatchesListPageTop100(
			DateSearchForm stockInSearchForm, ModelMap model) {
	...
	public String addAction(ModelMap model,
			RedirectAttributes redirectAttributes) {
		if (!authorize.isAllowed("INVVESI01")) {
	...
	public String editBatchSupply(@PathVariable String id, ModelMap model,
			RedirectAttributes redirectAttributes) {
		if (!authorize.isAllowed("INVVESI03")) {
	...
	private void loadStockInBatchForm(StockInForm stockInBatchForm,
		...
		if (stockInBatchForm.getId() != 0
			...
			// }
			stockInBatchForm.setDateOfSupply(stockInBatch.getSupplyDate()
					.toString());
	...
	private String displayStockInBatchFormPage(StockInForm stockInBatchForm,
		...
		stockinSearchForm.setDateOfRequest(stockInBatchForm.getDateOfSupply()
				.toString());
	...
	public String deleteBatchSupply(@PathVariable int id,
		...
		if (!authorize.isAllowed("INVVESI04")) {
		...
		try {
			...
			if (stockInBatch != null) {
				stockinSearchForm.setDateOfRequest(stockInBatch.getSupplyDate()
						.toString());
			}
	...
	public String newStockInLineItem(@PathVariable int batch_id,
			RedirectAttributes redirectAttributes, ModelMap model) {
		if (!authorize.isAllowed("INVVESIL1")) {
	...
	private void loadStockInLineItemForm(StockInLineForm stockInLineItemForm,
		...
		if (stockInLineItemForm.getId() != 0
			...
			if (globalItem != null) {
				...
				stockInLineItemForm.setGlobalitemname(String.valueOf(globalItem
						.getGlobalitemName()));
			}
			// set unit of measure
			...
			if (stockInLineItemForm.getManufacturedate() != null) {

				stockInLineItemForm.setManufacturedate(DateUtils
						.formatDateToString(stockInLineItem
								.getManufacturedate()));
			}

			if (stockInLineItemForm.getExpirydate() != null) {

				stockInLineItemForm.setExpirydate(DateUtils
						.formatDateToString(stockInLineItem.getExpirydate()));
			}

			stockInLineItemForm.setTinno(stockInLineItem.getTinno());
			stockInLineItemForm.setBatchno(stockInLineItem.getBatchno());
			stockInLineItemForm.setSerialno(stockInLineItem.getSerialno());
			stockInLineItemForm.setCost(stockInLineItem.getCost());
	...
	public String editStockInLineItem(
		...
		if (!authorize.isAllowed("INVVESIL3")) {
		...
		if (!result.hasErrors()) {
			System.out.print("Am inside no error");
			try {

				StockInLine stockInLineItem = this.stockInManager
						.editStockInLineItem(stockInLineItemForm);
				System.out.print("Finished editing");
				alert.setAlert(redirectAttributes, Alert.SUCCESS,
				...
				if (stockInBatch != null) {
					System.out.print("Returning" + stockInBatch.getId());
	...
	public String addLineSupplyItem(
		 ...
		 * stockInLineItemForm.getGlobalitemname()); System.out.print("itemids"
		 * + stockInLineItemForm.getGlobalItem());
		 * 
		 * 
		 * if (stockInLineItemForm.getGlobalitemname().equals("")) {
		 * alert.setAlert(redirectAttributes, Alert.WARNING,
		 * "Please Choose Global Item"); return
		 * "redirect:/inventory/stock_in/line_item/" + batchId;
		 * 
		 * 
		 * 
		 * }
	...
	public String deleteStockInLineItem(@PathVariable int id,
			...
			RedirectAttributes redirectAttributes) {
		if (!authorize.isAllowed("INVVESIL4")) {
