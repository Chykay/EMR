...
public class PointStockInController extends AbstractBaseController {
	...
	public String indexAction(ModelMap model,
		...
		if (!authorize.isAllowed("INVIUSI00")) {
	...
	public String addAction(ModelMap model,
			RedirectAttributes redirectAttributes) {
		if (!authorize.isAllowed("INVIUSI01")) {
	...
	public String editPointStockIn(@PathVariable String id, ModelMap model,
			RedirectAttributes redirectAttributes) {
		if (!authorize.isAllowed("INVIUSI03")) {
	...
	private void loadStockInBatchForm(PointStockInForm stockInBatchForm,
		...
		if (stockInBatchForm.getId() != 0
			...
			stockInBatchForm.setUnitId(stockInBatch.getUnitfrom()
					.getCategoryId());
			stockInBatchForm.setStockInDate(stockInBatch.getStockInDate()
					.toString());
	...
	private String displayStockInBatchFormPage(
			PointStockInForm stockInBatchForm, ModelMap model) {
		...
		stockinSearchForm.setDateOfRequest(stockInBatchForm.getStockInDate()
				.toString());
	...
	public String deleteBatchSupply(@PathVariable int id,
		...
		if (!authorize.isAllowed("INVIUSI04")) {
		...
		try {
			...
			DateSearchForm stockinSearchForm = new DateSearchForm();
			if (stockInBatch != null) {
				stockinSearchForm.setDateOfRequest(stockInBatch
	...
	public String newPointStockInLineItem(@PathVariable int batch_id,
			RedirectAttributes redirectAttributes, ModelMap model) {
		if (!authorize.isAllowed("INVIULI01")) {
		...
		StockInLineForm stockInLineItemForm = this.getPointStockInLineItemForm(
				id, batch_id);
		System.out.print("batch1" + stockInLineItemForm.getBatchId());
	...
	private void loadPointStockInLineItemForm(
		...
		System.out.print("First line loading..");
	...
	public String editStockInLineItem(@PathVariable String id,
			...
			ModelMap model) {
		if (!authorize.isAllowed("INVIULI03")) {
	...
	public String addLineSupplyItem(
		 ...
		 * "redirect:/inventory/point/stock_in/line_item/" + batchId;
		 * 
		 * 
		 * 
		 * }
		...
		if (!result.hasErrors()) {
			try {
				...
				System.out.print("batch2 before using it"
						+ stockInLineItemForm.getBatchId());
	...
	public String deleteStockInLineItem(@PathVariable int id,
			...
			RedirectAttributes redirectAttributes) {
		if (!authorize.isAllowed("INVIULI04")) {
