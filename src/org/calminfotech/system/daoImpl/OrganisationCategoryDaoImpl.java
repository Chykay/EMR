...
public class OrganisationCategoryDaoImpl implements OrganisationCategoryDao {
	...
	public List<OrganisationCategory> fetchTop50byOrganisation(int id) {
		...
		List<OrganisationCategory> list = sessionFactory
				.getCurrentSession()
				.createQuery(
						"from OrganisationCategoryVw  where  isDeleted=false ORDER BY Organisation_category_id DESC")
				// .setParameter(0, id)
	...
	public List<OrganisationCategory> fetchAll(int organisationid) {
		...
		List<OrganisationCategory> list = sessionFactory
				.getCurrentSession()
				.createQuery(
						"from OrganisationCategoryVw v where  v.isDeleted=false Order by v.categoryId DESC")
	...
	public OrganisationCategory getCategoryById(int categoryId) {
		...
		List<OrganisationCategory> list = sessionFactory
				.getCurrentSession()
				.createQuery(
						"from OrganisationCategory where categoryId = ? and organisation_id=? and is_deleted=false")
	...
	/*
	 * @Override public List<OrganisationCategoryOuterRecursive> fetchAllTypes()
	...
	public List<OrganisationCategoryList> fetchAllCatlistByOrganisation(
		...
		List<OrganisationCategoryList> list = sessionFactory
				.getCurrentSession()
				.createQuery(
						"from OrganisationCategoryList where organisationId = 1")
				// .setParameter(0, organisationid)
				.list();
	...
	public List<OrganisationCategory> fetchAllByOrganisationByCategoryType(
		...
		Criteria criteria = sessionFactory
				...
				.add(Restrictions.eq("organisationTypeId.organisationTypeId",
						cattypeid)).add(Restrictions.eq("isDeleted", false))
		 ...
		 * .
		 * createQuery("from OrganisationCategory where organisation_id=? and ")
