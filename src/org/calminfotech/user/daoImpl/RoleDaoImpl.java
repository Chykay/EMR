...
public class RoleDaoImpl implements RoleDao {
	...
	public Role getRoleById(int id) {
		List list = this.sessionFactory.getCurrentSession()
				.createQuery("from Role where roleId = ?").setParameter(0, id)
				.list();
		if (list.size() > 0) {
	...
	public List<Role> fetchAllRoleByOrganisation(Organisation organisation) {
		if (userIdentity.getOrganisation().getOrgCoy().getId() == 1) {
			List<Role> list = this.sessionFactory
					.getCurrentSession()
					.createQuery(
							"from Role where organisation.Id = ? and isDeleted=0 ")
	...
	public Role getRoleByAdmin(Organisation organisation) {
		Criteria criteria = this.sessionFactory
				...
				.add(Restrictions.eq("organisation.id", organisation
						.getOrgCoy().getId()));
